...
public class Cache implements Element {
	...
	public void handleRead(int addr,Element req) {
		...
		int setAddr,tagReq,wordAddr;

		if(lineSize > 1){
			wordAddr = Integer.parseInt(a.substring(32 - bitOffset,32),2);
		}
		else{
			wordAddr = 0;
		}
		...
		if(tag1 == tagReq) {
			...
			Simulator. getEventQueue().addEvent(
					...
					this ,req ,cach[setAddr*associativity].data[wordAddr] , cach[setAddr*associativity].data));				
		}
			...
			Simulator. getEventQueue().addEvent(
					new MemoryResponseEvent (
					Clock.getCurrentTime () ,
					this ,req ,cach[setAddr*associativity + 1].data[wordAddr],cach[setAddr*associativity + 1].data));				
	...
	public void handleWrite(int addr, int value,Element req) {
		...
		if(tag1 == tagReq) {
			...
			cach[setAddr*associativity].setDataElement(wordAddr,value);
			...
			cach[setAddr*associativity+1].setDataElement(wordAddr,value);
